...
stats: 585 likes · 77 retweets · 2336 bookmarks · 411370 views
tags: [agent-loop, orchestration, durable-execution, inngest, ai-infrastructure]
type: twitter-article
---

# The Agent Loop Architecture — @djfarrelly

**來源**：https://x.com/djfarrelly/status/2067677007140278630
**作者**：djfarrelly（@djfarrelly）
**發布日期**：2026-06-18
**收錄日期**：2026-06-19
**類型**：Twitter Article
**統計**：585 likes · 77 retweets · 2336 bookmarks · 411370 views
...
[圖 1 — 來源: Agent loop architecture system overview]
...
**TL;DR**：Agent loop 需要三層架構（Loop + Skill + Orchestrator）才能在生產環境中持久運行；`while True` 迴圈無法應對重啟、重試、並發與可觀測性需求。

### 核心論點

- **基礎設施問題，非提示問題**：loops 監督 loops、排程觸發、跨重啟存活、spawn 子 agent 等需求，是 infrastructure 問題，不是更好的 prompting 能解決的
- **三層架構**：Loop（cron + LLM 決策 + step 檢查點）、Skill（可重試的多步驟 durable workflow）、Orchestrator（排程/重試/並發/可觀測/熱部署引擎）
- **Step-level checkpointing 是省錢機制**：失敗時只重試失敗的 step，不重跑整個 workflow，避免重複 LLM 呼叫；10-30 個 agent 系統下效益顯著
- **Orchestration-aware agent**：agent 透過 SDK 工具直接寫新的 durable function 並即時注冊，sidecar 熱重載，無需重啟；skills 在 agent 進程死亡後持續運行
- **Review loop = 真正的「iterate」**：不是 magic，是 cron-triggered function 讀取 run history，LLM 評估是否誤報/閾值過敏，觸發 agent 更新 skill；`agent ephemeral, output durable`

### 關鍵數據 / 工具

- 框架：Inngest（`step.run()`、`step.invoke()`、`onFailure` hook）
- 參考 repo：https://github.com/inngest/utah（orchestration-aware agent harness）
- 並發控制語法：`concurrency: [{ limit: 1, key: "event.data.service" }]`
- Review loop 頻率：每週五 10am cron，讀 7 天 run history
- 585 likes · 2336 bookmarks（高收藏率，顯示實用性）

### 批判性觀察

- **Inngest 行銷色彩明顯**：架構原則有通用性，但所有 code examples 均使用 Inngest API，`utah` repo 仍是 internal test，生產驗證有限
- **Agent 自寫 skill 的驗證薄弱**：type check + agent 自測並非 bulletproof，review loop 是補救而非預防；文章對 safety/validation gate 著墨不深
- **「token capital compounding」論述稍虛**：Nadella 框架增添高度感，但 compounding 成立的前提（信號品質、review loop 準確率）未被量化論證
...
| 維度 | 分數 | 說明 |
|------|------|------|
| A. Workspace 可行動性 (30%) | 8/10 | 三層架構直接對應 agent harness 設計，code examples 可作藍本 |
| B. 創新性 (20%) | 7/10 | orchestration-aware agent 寫自身 skills 的 sidecar 模式是新穎綜合 |
| C. 證據品質 (20%) | 6/10 | 有具體代碼但均為 Inngest 示範，utah repo 仍是 WIP |
| D. 技術深度 (15%) | 8/10 | step-level checkpoint、onFailure hook、concurrency primitive 均有實質技術內容 |
| E. 泛化性 (15%) | 7/10 | 架構 pattern 與工具無關，但 examples 綁 Inngest |
| **加權總分** | **7.25/10** | 8×0.3+7×0.2+6×0.2+8×0.15+7×0.15 |

**整合決策**：Agent
**理由**：多步驟自主工作流（skill 撰寫、部署、自我評估迴圈）是 Agent 類型核心特徵，總分 7.25 高於門檻。
