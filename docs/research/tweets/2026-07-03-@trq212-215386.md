...
stats: 2673 likes · 212 retweets · 5976 bookmarks · 419839 views
tags: [fable5, agentic-coding, claude, prompting, unknowns, workflow]
type: twitter-article
---

# A Field Guide to Fable: Finding Your Unknowns — @trq212

**來源**：https://x.com/trq212/status/2073100352921215386
**作者**：Thariq（@trq212）
**發布日期**：2026-07-03
**收錄日期**：2026-07-03
**類型**：Twitter Article
**統計**：2673 likes · 212 retweets · 5976 bookmarks · 419839 views
...
**TL;DR**：Fable 5 agentic coding 的核心技能是在實作前/中/後系統性地發現並消除「未知數」，作者提供 7 個具體技術與 prompt 模板。

### 核心論點

- Fable 5 的瓶頸已從模型能力轉移到使用者「能不能說清楚自己不知道的事」
- 未知數分四類：Known Knowns / Known Unknowns / Unknown Knowns / Unknown Unknowns
- 過度具體 vs 過度模糊都會失敗；關鍵是對未知數有清醒認知
- 最有效的參考物是原始碼（Claude Design 讀 markup，不只看截圖）
- 用 implementation-notes.md 追蹤偏差，比事後 diff 更能學到教訓

### 關鍵數據 / 工具

- 模型：Claude Fable 5
- 工具：Remotion（影片 UI）、Whisper（逐字稿）、ffmpeg（剪輯）
- 實作案例：Fable 發佈影片全程由 Claude Code 剪輯
- HTML artifact 是視覺化 unknowns 的最佳載體
- Quiz 門檻：只有 100% 通過 quiz 才 merge

### 批判性觀察

- 7 個技術（Blind Spot Pass / Brainstorm / Interview / References / Implementation Plan / Implementation Notes / Quiz）提供的 prompt 範本可直接套用，學習成本低
- 文章未提供量化效益（節省多少來回次數）；體感說服力依賴信任作者身份
- 作者似為 Anthropic 內部人員（知道 Boris、Jarred、Claude Design 細節），insider 視角是資訊優勢也是可能的偏見來源
...
| 維度 | 分數 | 說明 |
|------|------|------|
| A. Workspace 可行動性 (30%) | 9/10 | 7 個技術含具體 prompt，可直接加入 SKILL |
| B. 創新性 (20%) | 8.5/10 | 4 象限 unknowns 框架用於 agentic coding 是新穎角度 |
| C. 證據品質 (20%) | 8/10 | 第一手 Fable 5 實戰（含 Fable launch video 案例），但無量化數據 |
| D. 技術深度 (15%) | 7.5/10 | 技術選型說明充分，但各技術的失敗案例不足 |
| E. 泛化性 (15%) | 8/10 | Fable 特定但 7 種技術可遷移至任何 LLM agentic 工作流 |
| **加權總分** | **8.3/10** | 9×0.3+8.5×0.2+8×0.2+7.5×0.15+8×0.15 |

**整合決策**：Skill
**理由**：7 個 prompt 技術可直接封裝為 SKILL（blindspot-pass / interview-unknowns / implementation-notes），高可行動性。
