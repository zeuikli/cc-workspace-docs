---
title: "8 个 Claude Code Hooks 实战（译） — @MinLiBuilds"
author: "实践哥MinLi（@MinLiBuilds）"
date: 2026-04-04
source: "https://x.com/MinLiBuilds/status/2040229845830091011"
type: tweet
---
...
**發布日期**：2026-04-04
**收錄日期**：2026-05-27（重新收錄，補全 15 個 MARKDOWN code block）
**類型**：Twitter Article
**統計**：181 likes · 34 retweets · 0 bookmarks · 53511 views
...
**TL;DR**：8 個 Claude Code Hooks 實戰（含完整 settings.json + bash 腳本）：PreToolUse 攔截危險命令/保護敏感檔案，PostToolUse 自動格式化/測試/Lint，Stop hook 自動 commit

### 核心論點

- **Hooks vs CLAUDE.md**：CLAUDE.md 只是建議（~80% 遵守率），Hooks 是強制觸發動作；設一次後台一直跑
- **Hook 8 個實戰**：①auto-format（Prettier/black/gofmt）②block-dangerous.sh（rm -rf/DROP TABLE/force-push，exit 2 攔截）③protect-files.sh（.env/.key/package-lock.json）④PostToolUse 測試⑤PR 前強制測試通過⑥auto-lint（ESLint）⑦command audit log（精確到秒）⑧Stop hook 自動 commit
- **exit code 語意**：exit 2 = 攔截並告知 Claude 改策略，exit 0 = 放行
- **完整 settings.json**：PreToolUse（Bash + Edit/Write + PR）+ PostToolUse（Write/Edit）+ Stop，可直接複製部署
- **Auto-commit 爭議**：與 `claude -w feature-branch`（worktrees）搭配使用，每任務獨立分支；作者認為 git 歷史因此變可讀

### 關鍵數據 / 工具

- `block-dangerous.sh`：攔截 rm -rf / git reset --hard / git push --force / DROP TABLE / curl|sh / wget|bash
- `protect-files.sh`：保護 .env* / .git/* / package-lock.json / yarn.lock / *.pem / *.key / secrets/*
- `require-tests-for-pr.sh`：matcher `mcp__github__create_pull_request`（MCP 工具名稱為觸發條件）
- `log-commands.sh`：`printf '%s %s
' "$(date -Is)" "$cmd" >> .claude/command-log.txt`
- `auto-commit.sh`：matcher `Stop`（session 結束時觸發）
- Boris Cherny（Claude Code 創建者）引用：Hooks 實時反饋讓輸出品質提升 2-3 倍
- 部署：chmod +x .claude/hooks/*.sh + .gitignore command-log.txt

### 批判性觀察

- **block-dangerous.sh pattern 不完整**：未涵蓋 `truncate`、`dd`、`mkfs` 等危險命令；生產環境建議結合 allowlist 而非 denylist
- **auto-commit 的 commit message 品質問題**：固定用 `chore(ai): apply Claude edit` 會讓 git log 失去語意，需搭配 squash merge 才實用
- **ESLint hook 有副作用**：`$(jq -r '.tool_input.file_path')` 在 file_path 含空格時會失敗；應加引號
...
| 維度 | 分數 | 說明 |
|------|------|------|
| A. Workspace 可行動性 (30%) | 10/10 | 15 個 code block 全部可直接複製部署到 .claude/hooks/，cc-workspace 已有同類 hooks |
| B. 創新性 (20%) | 7/10 | 整理型文章，但 MCP tool matcher（`mcp__github__create_pull_request`）和 Stop hook 細節有參考價值 |
| C. 證據品質 (20%) | 8/10 | 完整可執行的 JSON + bash code，Boris Cherny 引用增加可信度 |
| D. 技術深度 (15%) | 8/10 | exit 2 攔截機制、matcher 語法、worktrees 整合、audit log 設計均有技術深度 |
| E. 泛化性 (15%) | 9/10 | 跨語言（Node/Python/Go/Rust），適用所有 Claude Code 用戶 |
| **加權總分** | **8.75/10** | 10×0.3+7×0.2+8×0.2+8×0.15+9×0.15 |

**整合決策**：SKILL
**理由**：完整 hooks 套件（block-dangerous + protect-files + auto-commit + audit-log）可直接整合為 workspace 的 hooks-bootstrap SKILL
