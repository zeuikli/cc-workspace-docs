---
title: "Lessons from Building Claude Code: How We Use Skills — @trq212"
...
# Lessons from Building Claude Code: How We Use Skills — @trq212

**來源**：https://x.com/trq212/status/2033949937936085378
**作者**：Thariq（@trq212）
**發布日期**：2026-03-17
**收錄日期**：2026-05-10
**類型**：Twitter Article
**統計**：16367 likes · 2287 retweets · 44019 bookmarks · 6854215 views
...
**TL;DR**：Anthropic 內部數百個 skills 實戰總結——9 種類型分類框架 + Gotchas 優先 + Skill-as-folder 心智模型，是目前最權威的 Claude Code skill 設計指南。

### 核心論點

1. **Skill 不是 markdown 檔，是資料夾**：可含 scripts、assets、data、config.json、hooks，最強的 skills 都善用這個結構做 progressive disclosure
2. **9 種類型分類**：Library/API Reference、Product Verification、Data Fetching、Business Process、Code Scaffolding、Code Quality、CI/CD、Runbooks、Infrastructure Ops — 可用此框架審查 workspace 是否有缺口
3. **Gotchas section 是最高信號密度的內容**：從真實失敗點積累，持續更新；比功能描述更有價值
4. **Description field 是給模型看的觸發詞**：Claude Code 啟動時掃描 description 決定呼叫哪個 skill，寫法應描述「何時觸發」而非「功能摘要」
5. **On-demand hooks**：skill 被呼叫時才啟用、session 結束後消失，適合高度主觀的 guardrails（如 /careful 阻擋 DROP TABLE）

### 關鍵數據 / 工具

- Anthropic 內部有數百個 skills 在 active use
- `CLAUDE_PLUGIN_DATA` 環境變數：skill 升級後的穩定資料存放路徑
- Skill Creator 工具：新發布，用於加速 skill 建立
- PreToolUse hook：Anthropic 用來追蹤 skill 使用率，偵測 undertriggering
- Plugin marketplace（内部）：超過一定規模後的分發機制

### 批判性觀察

1. **缺乏量化佐證**：「數百個 skills」、「Gotchas 最高信號」等主張基於內部觀察，無定量實驗；實際效果因 skill 品質差異很大
2. **Skill 組合依賴管理未解決**：作者明確說「dependency management is not natively built in」，只能靠 name reference；大型 marketplace 的版本控制/衝突問題留白
3. **Marketplace 治理門檻模糊**：「有 traction 就可以 PR」的草根機制適合 Anthropic 內部文化，但無法直接套用到有合規要求的外部組織
...
| 維度 | 分數 | 說明 |
|------|------|------|
| A. Workspace 可行動性 (30%) | 9/10 | 9 種類型直接對映 cc-workspace 缺口稽核；Gotchas 設計、on-demand hooks、config.json 模式均可立即套用 |
| B. 創新性 (20%) | 7/10 | Skill-as-folder、description=觸發詞、on-demand hooks 是新視角；但多數內容是最佳實踐彙整而非突破性概念 |
| C. 證據品質 (20%) | 7/10 | 來自 Anthropic 內部數百個 skills 的第一手觀察，有具體例子；缺乏定量實驗數據 |
| D. 技術深度 (15%) | 8/10 | 有具體技術細節（CLAUDE_PLUGIN_DATA、hooks 設計、PreToolUse 使用率追蹤），可直接參照實作 |
| E. 泛化性 (15%) | 9/10 | 9 種類型框架跨 codebase/org 通用；Gotchas 優先原則適用所有 skill 設計 |
| **加權總分** | **8.05/10** | 9×0.3 + 7×0.2 + 7×0.2 + 8×0.15 + 9×0.15 = 2.70+1.40+1.40+1.20+1.35 |

**整合決策**：Rule
**理由**：核心是通用 skill 設計原則（9 種類型框架 + Gotchas 優先 + Skill-as-folder），直接補充到 `.claude/refs/skill-authoring.md`
