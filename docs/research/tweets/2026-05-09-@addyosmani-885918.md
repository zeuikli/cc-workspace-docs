...
stats: 1343 likes · 162 retweets · 3007 bookmarks · 180365 views
type: twitter-article
---

# Agent Harness Engineering — @addyosmani

**來源**：https://x.com/addyosmani/status/2053231239721885918
**作者**：Addy Osmani（@addyosmani）
**發布日期**：2026-05-09
**收錄日期**：2026-05-10
**類型**：Twitter Article
**統計**：1343 likes · 162 retweets · 3007 bookmarks · 180365 views
...
Harness engineering 是將 agent 失敗轉化為永久性配置改進的工程紀律，harness 品質決定 agent 實際表現，模型只是輸入之一。

### 核心論點

1. **Harness 決定體驗，模型只是輸入**：Claude Code、Cursor、Codex 底層模型可能相同，但 harness 差異決定用戶體驗的根本差距。
2. **Ratchet 原則**：每一次 agent 失敗都應轉化為永久性規則（AGENTS.md 更新、hook 新增、subagent 調整），而非 retry 後遺忘。
3. **失敗是配置問題，不是模型問題**：HumanLayer 的核心主張——與其等下一版模型，不如工程化解決當前失敗。
4. **組件需有明確職責**：無法命名某組件存在目的 → 移除；十個專注工具 > 五十個重疊工具。
5. **Harness 不會消失，只會轉移**：模型升級讓舊 scaffolding 過時，同時開啟新失敗模式，需要新的 scaffolding。

### 關鍵數據 / 工具 / 引用

- `Agent = Model + Harness`（@Vtrivedy10 定義）
- 同一模型在 off-the-shelf framework vs 客製化 harness 的 benchmark 差距「drastically lower」
- Claude Code 架構：Fareed Khan 的 breakdown 作為成熟 harness 的公開參考
- HumanLayer 框架：「It's not a model problem. It's a configuration problem.」
- ReAct 迴圈：reason → act via tool call → observe → repeat
- HaaS（Harness-as-a-Service）：從 LLM API（completion）到 Harness API（runtime）的產業轉移
- Flue（@FredKSchott）：推薦的 agent harness 框架

### 批判性觀察

1. **缺少量化數據**：「drastically lower benchmark」等主張未附具體數字或實驗連結，可信度依賴讀者對 Addy 的信任，而非可重現的測試。
2. **Ratchet 原則的維護成本被低估**：每次失敗都加規則，長期累積可能讓 AGENTS.md / system prompt 膨脹至難以維護，文章未討論「何時移除過時規則」的判斷標準（雖有提「當模型改善時移除」，但標準模糊）。
3. **HaaS 趨勢分析與實操建議脫節**：文章後半段轉向產業觀察，但對工程師的具體行動指引（如何判斷何時採用 HaaS vs 自建 harness）未給出決策框架。
...
| 維度 | 分數 | 理由 |
|------|------|------|
| A. Workspace 可行動性 | 9/10 | Ratchet 原則、Hook 生命週期、progressive disclosure、PGE splits 等概念可直接對應 cc-workspace 現有設計（hooks/、AGENTS.md、harness-eval skill）；能立即審視並強化既有配置 |
| B. 創新性 | 7/10 | 「Harness engineering」命名框架為產業共識的清晰化整合，非全新概念；HaaS 方向有洞見；但 cc-workspace 已有大量相關實踐（subagent-strategy.md、harness-design.md），增量不高 |
| C. 證據品質 | 5/10 | 主要為第一方觀點彙整（Trivedy、HumanLayer、Fareed Khan 架構圖），缺乏可量化的 benchmark 數據或可重現實驗；說服力依賴作者權威性 |
| D. 技術深度 | 7/10 | 組件分解詳細（filesystem/bash/sandbox/memory/hooks/loops/planning/splits），但多數停留在概念說明層次，缺少配置範例或實際代碼 |
| E. 泛化性 | 9/10 | 框架適用所有 coding agent（Claude Code、Cursor、Codex 等），跨 harness 通用；「ratchet」與「失敗→規則」的方法論可直接移植任意 workspace |
| **加權總分** | **7.35/10** | 9×0.3 + 7×0.2 + 5×0.2 + 7×0.15 + 9×0.15 = 2.70 + 1.40 + 1.00 + 1.05 + 1.35 = **7.50** |

**整合決策**：Rule（核心是通用原則，適用所有任務）
**整合位置**：強化 `.claude/rules/core.md` 的「Ratchet 原則」段落 + `.claude/refs/harness-design.md` 補充 HaaS 趨勢
**整合狀態**：待實作
