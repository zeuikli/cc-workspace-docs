---
url: "https://x.com/PrajwalTomar_/status/2074787074641899896"
title: "Skill 蔓延與必要性證明：uninstall anything that overlaps or never triggers"
author: "Prajwal Tomar"
archived: 2026-07-20
handle: "@PrajwalTomar_"
published: 2026-07-08
stats: 42 likes · 9 retweets · 20973 views（主推）· 158 likes · 152607 views（引用 Article）
tags: [skill-sprawl, organic-deprecation, context-budget, harness-integrity, tool-bloat]
type: tweet+quoted-twitter-article
---

# Skill 蔓延與必要性證明 — @PrajwalTomar_

**來源**：https://x.com/PrajwalTomar_/status/2074787074641899896
**引用 Article**：https://x.com/PrajwalTomar_/status/2074095215422705805（"These 10 Skills Turn Claude Code Into an ENTIRE Team."）
**發布日期**：2026-07-08 · **收錄日期**：2026-07-20
...
Paste the whole thing into your session and say:

"Read this article end to end. Then list every skill I currently have installed, and compare it against this list. Uninstall anything that overlaps, never triggers, or is a worse version of something in the article. Install the replacements. Then give me a before and after count of what my context window is carrying."

That's it. Claude reads the breakdown, audits your own workflow, and builds your setup for you.

And because it skips what you don't need, you dodge the trap most people are in right now: 300 installed skills fighting for context.

Articles aren't content anymore. They are playbooks your agent runs for you.

---

## 引用 Twitter Article 全文（These 10 Skills Turn Claude Code Into an ENTIRE Team.）
...
1. Skill Creator - the skill that builds every other skill
...
2. Superpowers - a senior engineer's process, forced
...
4. Context Mode - stop burning your context window
...
6. Karpathy Rules - the discipline layer
...
7. Impeccable - kills the AI-generated look
...
8. Anthropic Document Skills - real files, not just code
...
9. /watch - Claude Code can now watch videos
...
10. /code-review and /code-review ultra - the ship gate
...
### 核心論點
1. **skill hoarding 是主動傷害非中性囤積**：Claude 載入 skill 內容有 context 上限（文中引 ~15,000 chars），每個安裝的 skill 都在競爭這個空間——300 skills = 稀釋到全部都觸發不良（「300 installed skills fighting for context」）。
2. **可操作的修剪判準三分**：uninstall anything that ① **overlaps**（與他 skill 重疊）② **never triggers**（從不觸發）③ **is a worse version**（有更好替代）。
3. **before/after count 儀式**：修剪要以「context window 攜帶量前後對比」量化收尾——不是感覺變乾淨，是數字變小。
4. **文章即 playbook**：把稽核文章直接餵給 agent 自我稽核（"Articles aren't content anymore. They are playbooks your agent runs for you."）——read-path 思想的另一面：知識要被 agent 執行而非被人閱讀。

### 侷限
- 判準 ②「never triggers」在無 usage telemetry 時只能靠印象——作者未提供量測機制，預設讀者用 LLM 自查（自報 = 中間態，本 workspace 鐵律會要求收據）。
- 引用 Article 主體是 10-skill 推薦清單（行銷向），價值集中在前段的 sprawl 診斷與修剪 prompt，後段清單時效性低。

## 5 維度評分

| 維度 | 分數 | 理由 |
|---|---|---|
| 資訊密度 | 3/5 | 核心洞見集中前 1/3；後段清單填充 |
| 可操作性 | 5/5 | 修剪 prompt 可直接執行、三判準明確、before/after 量化收尾 |
| 佐證強度 | 2/5 | 15k chars 上限與 300-skill 稀釋皆未附量測；經驗性主張 |
| 與本 workspace 相關性 | 5/5 | 直接命中 organic deprecation（SPEC S5）+ 31 skills 必要性證明的當前議題 |
| 新穎性 | 3/5 | sprawl 診斷已見於多源；「文章即 playbook」框架較新 |

**總評：18/25 — 收錄**（可操作性與相關性補足佐證薄弱）

## 與本 workspace 現況對應

| 推文主張 | 本 workspace 對應 | 差距 |
|---|---|---|
| uninstall never-triggers | task-log `skills_used`（tracker 通用捕捉）→ skill-optimizer organic deprecation | **我們有真 telemetry，不必靠 LLM 自查印象**——但資料 2026-07-20 才開始流動，需累積窗口才能判 |
| uninstall overlaps | CONCEPT-MAP §6「疑似重疊 12 對」已盤點（多為維度差異非真重疊） | 已有地圖，判準待 usage 資料佐證 |
| before/after count | byte ceiling 量測（core.md §Framework Integrity） | 已常態化，比推文更嚴（healthcheck 強制） |
| 文章即 playbook | dreaming → PLAYBOOK.md（情境→行動） | 同構；我們多了 provenance gate |

**結論**：本 workspace 的做法**嚴格超集**於推文建議——推文靠 LLM 印象自查，我們有確定性 telemetry + 閘門。缺的只是資料累積時間（skills_used 剛通）。反向教訓：不要在資料窗口不足時急著砍（「never triggers」的觀察期要明訂，否則新 skill 全中槍）。
