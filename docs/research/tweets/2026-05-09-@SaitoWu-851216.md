---
url: "https://x.com/SaitoWu/status/2053101671035851216"
title: "AI 编程的底层原则"
author: "Saito"
archived: 2026-06-09
handle: "@SaitoWu"
published: 2026-05-09
stats: "👍 189 · 🔁 33 · 🔖 437 · 👁 31,980"
type: twitter-article
---

# AI 编程的底层原则

> **作者**: [@SaitoWu](https://x.com/SaitoWu) (Saito)  
> **原文**: https://x.com/SaitoWu/status/2053101671035851216  
> **發布日期**: 2026-05-09  
> **類型**: Twitter Article  
> **統計**: 👍 189 · 🔁 33 · 🔖 437 · 👁 31,980

---

## TL;DR

AI 時代軟體工程基礎知識比以往更重要——壞代碼在 AI 加持下熵爆炸更快，工程師的核心職責從「寫代碼」轉向「守住設計邊界」。
...
想直接抄作业的同学，可以去 GitHub 搜 [MattPocockSkills](https://github.com/mattpocock/skills)。里面有他提到的所有 skill：Grill Me、Ubiquitous Language、Improve Codebase Architecture。

他也在 AIhero.dev 写 newsletter，YouTube 和 Twitter 都持续更新。

完整演讲逐字稿，可以在 [Podwise](https://podwise.ai/dashboard/episodes/7846761) 免费获取。
...
1. **壞代碼 AI 時代更貴**：在好代碼庫 AI 發揮 10x；在爛代碼庫 AI 只會疊加混亂，entropy 爆炸
2. **Grill Me 技術**：先建立 shared design concept（40-100 問），再寫代碼；比 Plan Mode 更重要的是概念對齊
3. **Ubiquitous Language**：掃描代碼庫提取術語→Markdown 表格→每個 prompt 帶上，縮短 AI 思考痕跡、提升精準度
4. **TDD 強制反饋循環**：速率限制 = 反饋速率，強制 small deliberate steps 避免錯誤疊加
5. **深模組 vs 淺模組**：Deep modules（功能多+接口極簡）是 AI 友好結構；工程師職責是守住接口邊界，把實現交給 AI

### 關鍵數據 / 工具 / 引用

- **GitHub**: https://github.com/mattpocock/skills（13k+ stars）
- **書籍引用**：Brooks《The Design of Design》、Ousterhout《A Philosophy of Software Design》、《The Pragmatic Programmer》
- **Kent Beck**：每天都要投資系統設計
- **Specs-to-code 親測**：第 1 次 OK → 第 2 次差 → 第 3 次垃圾（質量線性遞降）
- **Podwise 逐字稿**：https://podwise.ai/dashboard/episodes/7846761

### 批判性觀察

1. **論點缺乏量化**：「10x 效率」「40-100 個問題」等說法無 benchmark 佐證，主要是定性觀察，可重現性待驗證
2. **Grill Me vs Plan Mode 對比過度簡化**：稱 Plan Mode「太急著產出」但未說明具體測試條件，這個結論對不同任務類型可能有差異
3. **深模組原則的實踐難度被低估**：如何判斷「接口夠簡」需要豐富設計經驗，初階工程師可能難以拿捏邊界
...
| 維度 | 分數 | 理由 |
|------|------|------|
| A. Workspace 可行動性 | 9/10 | 3 個 Skill 有具體 prompt 或步驟可直接套用；GitHub repo 可參照 |
| B. 創新性 | 7/10 | Grill Me、Ubiquitous Language 是具體新工具；深模組框架應用到 AI coding 有力，但「好代碼重要」整體論點通用 |
| C. 證據品質 | 7/10 | 親測觀察 + 書籍引用 + 13k stars 社群驗證，但缺乏量化 benchmark |
| D. 技術深度 | 8/10 | 各 skill 有完整步驟；TDD + 靜態類型三板斧明確；Improve Codebase Architecture 流程清晰 |
| E. 泛化性 | 9/10 | 適用任何 AI coding 工具，跨技術棧；DDD、TDD、深模組均為通用原則 |
| **加權總分** | **8.05/10** | 9×0.3 + 7×0.2 + 7×0.2 + 8×0.15 + 9×0.15 |

**整合決策**：SKILL（核心是靜態提示詞/方法論）  
**整合位置**：可參照 https://github.com/mattpocock/skills 實作 `grill-me`、`ubiquitous-language` 兩個 skill  
**整合狀態**：待實作
