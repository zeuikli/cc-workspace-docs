...
## 內建 CLI 工具指令（W14–W17 新增）

### `/powerup`（W14）

內建動畫 demo 的互動式功能教學課程，直接在 terminal 中學習 Claude Code 各項功能：

```text
> /powerup
```

### `/usage` Breakdown（W16–W17）

顯示用量驅動因素分析：parallel sessions、subagents、cache misses、long context，各附百分比 + 優化提示。`d`/`w` 切換 day / week 視圖：

```text
> /usage    # d = day view, w = week view
```

W17 更新：`/cost` 和 `/stats` 合併入 `/usage`，舊名稱仍可用（自動跳到對應 tab）。
