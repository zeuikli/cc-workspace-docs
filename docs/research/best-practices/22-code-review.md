...
以下指令與 Code Review 產品互補，針對 PR 生命週期的不同環節提供自動化。
...
不需開瀏覽器，從 terminal 一鍵對當前分支的 PR 啟動 Auto-Fix：

```text
> /autofix-pr
```

### `/ultrareview`（W16 v2.1.111，W17 公開 Preview）
...
W17 進入 public research preview，launch dialog 顯示 diffstat。

### `claude ultrareview`（CLI 子命令，W18）

非互動式的 `/ultrareview`，可在 CI/scripts 中使用：

```bash
claude ultrareview            # 輸出結構化結果
claude ultrareview --json     # 輸出原始 JSON
# exit 0 成功，exit 1 失敗
```

### `/simplify`（2026-02-28，Boris Cherny）

PR 完成後對已變更程式碼進行 review for reuse / quality / efficiency，自動精煉並提交改善。Boris Cherny 個人日常使用驗證：

```text
> /simplify
```

適用時機：PR 草稿完成後發 PR 之前、收到「這段能否簡化」的 review 意見後。
