...
> **更新方法**：overnight-research 全網搜尋 + 官方 code.claude.com 文件驗證  
> **資料截止**：2026-05-23（Claude Code v2.1.150）

### 最新版本狀態

**當前最新版本：v2.1.150（2026-05-23）**

近期發布時間線：
- v2.1.150（2026-05-23）：內部基礎設施改進
- v2.1.149（2026-05-22）：`/usage` 分類明細、GFM task list Markdown 支援、重大安全修復
- v2.1.147（2026-05-21）：Pinned sessions、`/code-review`（取代 `/simplify`）
- v2.1.139–142（2026-05-11–15）：Agent view、`/goal`、Fast mode on Opus 4.7

---

### Week 13–20 新增核心功能（2026-03-23 ～ 2026-05-15）

#### Auto Mode（Week 13，v2.1.83，研究預覽）

新的中間層權限模式，介於手動確認與 `--dangerously-skip-permissions` 之間：
- 分類器自動處理安全操作（不中斷）、阻擋高風險操作
- Week 19 新增 **Hard Deny Rules**（`settings.autoMode.hard_deny`）：無論任何 allow 例外，指定操作**絕對不自動執行**
- 適合：CI/CD 管道、無人值守任務

#### Computer Use（Week 14，研究預覽）

- Claude 可直接在 CLI 開啟原生應用程式、點選 GUI、驗證介面變更
- 閉合 GUI-only 驗證的缺口（測試端到端流程）

#### Ultraplan（Week 15，早期預覽）

- 從 CLI 在雲端起草計劃、在 Web Editor 審閱並評論
- 可遠端執行或拉回本地；首次運行自動建立雲端環境

#### `/ultrareview`（Week 17，公開研究預覽）

- 雲端 bug-hunting agent fleet 遠端運行
- 發現自動同步回 CLI 或 Desktop
- `claude ultrareview`（Week 18）：可在 CI scripts 和一次性執行中使用

#### Claude Opus 4.7 + xhigh 努力等級（Week 16）

- Opus 4.7 成為 Max/Team Premium 的新預設模型
- 新增 `xhigh` 努力等級：大多數編程任務的**推薦設定**
- `/effort` 互動式滑桿可即時調整
- 環境變數：`$CLAUDE_EFFORT`（hooks 和 Bash tool 均可讀取）

#### Agent View（Week 20，研究預覽）

```bash
claude agents
```

- 單螢幕管理所有 Claude Code sessions：運行中 / 等待輸入 / 已完成
- 可在任一 row 附加進入完整對話，按 `←` 返回列表
- 背景 session 在無 terminal 附加時繼續運行
- 新增 dispatch flags：`--add-dir`、`--settings`、`--mcp-config`、`--plugin-dir`、`--permission-mode`、`--model`、`--effort`、`--dangerously-skip-permissions`
- `claude agents --json`：以 JSON 列出 live sessions（供腳本/tmux 使用）
- `claude agents --cwd <path>`：限定列出指定目錄的 sessions

#### `/goal` 命令（Week 20，v2.1.139）

```
> /goal all tests in test/auth pass and the lint step is clean
```

- 設定完成條件，Claude 跨多個 turns 自動工作直到條件成立
- 每個 turn 後由快速模型檢查條件是否滿足；不滿足則繼續而非交還控制權
- 適合：有可驗證終止狀態的複雜任務（模組遷移、測試修復）
- 支援 interactive、`-p`、Remote Control 模式

#### Fast Mode on Opus 4.7（Week 20）

- `/fast` 現在預設在 **Opus 4.7** 執行（原為 Opus 4.6）
- 同等模型品質，約 **2.5×** 速度提升，每 token 成本較高
- 固定 Opus 4.6：設 `CLAUDE_CODE_OPUS_4_6_FAST_MODE_OVERRIDE=1`

#### Rewind 選單增強（Week 20）

- 新增「Summarize up to here」：壓縮較早的 context 同時保留近期 turns

#### Plugins 增強（Week 19）

- `--plugin-dir` 現接受 `.zip` 格式
- `--plugin-url`：從 URL 直接抓取 plugin archive（無需預先安裝）
- `claude plugin details <name>`：顯示 plugin 組件清單 + 預估每 session token 成本
- 根目錄有 `SKILL.md` 且無 `skills/` 子目錄的 plugin 自動識別為 skill

#### Windows 原生支援（Week 18）

- **不再需要 Git for Windows**
- PowerShell 成為 Bash 不存在時的預設 shell tool
- 正確處理 `PWD`/`OLDPWD` 環境變數

#### 記憶體與快取優化（v2.1.128）

- Sub-agent progress summaries 命中 prompt cache：**cache creation token 成本降低約 3×**
- 1M context autocompact 修復（避免誤報「prompt too long」）
...
```json
// 新的 exec 形式：直接 spawn，不透過 shell（路徑佔位符無需引號）
"args": ["string[]"]

// PostToolUse 的 continueOnBlock 選項
// 當 hook 拒絕時，將拒絕原因回饋 Claude 並繼續 turn（而非結束）
"continueOnBlock": true

// hooks 可發出桌面通知、視窗標題、bell，無需控制 terminal
"terminalSequence": "..."
```

完整 27 個事件類別：
- **Session**：SessionStart、Setup、SessionEnd、ConfigChange、CwdChanged
...
- **Advanced**：Elicitation、ElicitationResult、Notification、Stop、StopFailure

---

### MCP 生態系統狀態（2026-05-23）

- **官方 registry：9,400+ servers**（跨 OpenAI、Google、主要 IDE）
- SSE Transport **已棄用**，改用 HTTP servers
- 新功能：`elicitation`（MCP servers 可在任務中請求表單輸入）
- `claude mcp serve`：將 Claude 的工具暴露給其他 MCP clients
- `alwaysLoad: true`：豁免 server 的 deferred loading（適合高優先工具）
- Sub-agent 的 MCP stdio servers 現在收到 `CLAUDE_PROJECT_DIR`

---

### Breaking Changes / 棄用（2026年5月）

| 項目 | 狀態 | 遷移建議 |
|------|------|---------|
| **TaskOutput Tool** | 已棄用 | 改用 `Read` 直接讀取 background task 的輸出路徑 |
| **Claude Haiku 3** | 已退役 | 遷移至 `claude-haiku-4-5-20251001` |
| **Claude Sonnet 4 (`claude-sonnet-4-20250514`)** | 預計 6/15 退役 | 遷移至 Sonnet 4.5+ |
| **Claude Opus 4 (`claude-opus-4-20250514`)** | 預計 6/15 退役 | 遷移至 Opus 4.5+ 或 4.7 |
| **SSE Transport（MCP）** | 已棄用 | 使用 HTTP servers |
| **`/simplify` 命令** | 已替換 | 改用 `/code-review` |
| **API Key 模式下的 Remote/Schedule 功能** | 停用 | 設置 `ANTHROPIC_API_KEY` 時，RemoteControl、`/schedule`、Claude.ai MCP connectors 停用；卸除 API key 恢復 |

---

### 安全修復（v2.1.149，2026-05-22）

1. **PowerShell 權限繞過**：內建 `cd` 函數可偷改目錄而不被偵測到
2. **Git Worktree Sandbox 繞過**：write allowlist 錯誤覆蓋整個主 repo 而非只有 `.git/`
3. **PWD 變數過期**：`PWD`/`OLDPWD`/`DIRSTACK` stale 追蹤造成權限分析缺口
4. **SOCKS5 Null-Byte 注入**（2026-05-20 回報）：hostname null-byte 注入可繞過 sandbox allowlist — **已修復**
...
> **方法**：對照 `.claude/rules/`（4 個 auto-load 規則）+ 本地 `.claude/refs/`（30 個參考文件）現狀逐項驗證；WebSearch 不可用，以 `2026-05-25-papers-analysis.md`（84 篇論文深度分析）補充新發現。

### 已落地的建議（✅）

**第一章：CLAUDE.md 與記憶系統**
- ✅ CLAUDE.md ≤ 200 行上限：`core.md` 已實作，workspace CLAUDE.md ~198 行在預算內
- ✅ 雙層記憶架構（Auto Memory + MEMORY.md）：`core.md` §長期記憶回路 已落地
- ✅ 「移除這行，Claude 會犯錯嗎？」剪枝測試：`core.md` §Framework Integrity 已明文
- ✅ Path-scoped rules（`.claude/rules/`）：4 個 auto-load + 1 個 path-scoped `security-hygiene.md` 已實作

**第二章：Hooks**
- ✅ PreToolUse 防守型 hook：`block-dangerous.sh` 已實作
- ✅ PostToolUse 格式化 hook：`post-edit.sh` 已實作
- ✅ Conditional Hooks `if` vs `matcher`（G2）：`hooks-decision-tree.md` 已補充
- ✅ `$CLAUDE_EFFORT` 環境變數（G4）：`hooks-decision-tree.md` §W19 已補充
- ✅ PreCompact hook `additionalContext`（G3 PARTIAL）：`pre-compact.sh` 已實作語義感知 compact
- ✅ 27 個 hook 事件（含 `continueOnBlock`）：`2026-05-17` 報告已記錄

**第三章：Prompt Caching**
- ✅ Cache 三禁止（mid-session 切換模型/工具/CLAUDE.md）：`context-management.md` 已實作
- ✅ Compact 三層觸發（行為信號 > 數字閾值 > 定時器）：`context-management.md` 已落地
- ✅ 動態資訊用 `<system-reminder>` 注入 messages（不改 system prompt）：本報告附錄 A.5 有範本

**第四章：Subagent**
- ✅ 委派五條件 + Fan-out 上限 4：`subagent-strategy.md` 已落地
- ✅ child 輸出只含結果（不加「完成」「Done.」）：`subagent-strategy.md` 已明文
- ✅ Advisor 模式（架構決策前 / 完成前諮詢）：`subagent-strategy.md` 已落地
- ✅ Prompt Injection 防護 `<untrusted_objective>`：`subagent-strategy.md` 已落地

**第五章：Skills**
- ✅ API 最大 Skill 數量 8 個：本報告 §5.5 已記錄

**第六章：MCP**
- ✅ `defer_loading: true`（Tool Search）：本報告 §6.4 已記錄
...
**第十章：驗證**
- ✅ PGE 原則（Generator ≠ Evaluator）：`core.md` §PGE + R12 Fail Loud 已落地
- ✅ 展示前 5 / 後 5 行輸出：`core.md` 已明文，禁口頭「測試通過」

### 待追蹤的 gap（⚠️）

- ⚠️ **G3 PARTIAL 未完全落地**：`pre-compact.sh` 已實作 `additionalContext`，但 PreCompact hook `exit 2` **阻斷模式**（完全阻止 compact 執行）尚未在任何文件或 hook 中實作。若有需保護不被 compact 的關鍵決策，此路徑未覆蓋。
- ⚠️ **Routines 使用限額資訊可能過時**：本報告 §8.4 記載 Pro=5次/日、Max=15次/日，須對照最新 Anthropic 定價頁確認（Claude Code 版本持續迭代）。
- ⚠️ **`AgentOpt` 強模型當 Planner 反直覺數據**：本報告 §4.2 引用「31.71% vs 74.27%」，來源為引用資料，未在本 workspace 自測驗證（信度標記應為 [C]，非 [O]）。
- ⚠️ **`xhigh` 努力等級未在 `subagent-strategy.md` 規範化**：本報告 §9.2 提到 `xhigh` 是 agentic 編碼推薦預設，但 `subagent-strategy.md` 的模型選擇表格未包含 effort level 維度，屬待補 gap。
- ⚠️ **`TaskOutput` 棄用後的 subagent 輸出模式**：本報告 §4.4 仍描述舊的 worktree 流程；新版應用 `Read` 替代 `TaskOutput`（已在 2026-05-17 報告 §GOTCHA #39 補充，但本報告未同步）。

### 新發現的最佳實踐補充（🆕）

- 🆕 **Harness Engineering > 模型選擇**：2026-05-25 論文分析（84 篇）確認「Harness 配置效益超越模型升級」。本報告第四章建議模型分層（Haiku/Sonnet/Opus），但未強調 harness 設計本身（CLAUDE.md 品質 + Hooks 架構 + Memory 設計）是更大的槓桿點。引用：AgentFlow harness 使同等模型達 84.3% vs 其他 harness 的顯著低分。
- 🆕 **`/goal` 命令（Week 20）**：設定可機械性驗證的完成條件後，Claude 跨多輪自動工作直到條件成立。與本報告 §10.1 的「成功條件開工前定義」直接互補，但 `core.md` 尚未提及此命令作為實現 R4 的工具。
- 🆕 **`claude agents`（Agent View）**：單螢幕管理所有 Claude Code sessions，含 `--json` 機器可讀格式。本報告 §4.4 的「平行 worktree 工作流」在 Agent View 下操作效率大幅提升，是對 §4.4 的原生工具補充。
- 🆕 **Sub-agent progress summaries 命中 prompt cache**（v2.1.128）：cache creation token 成本降低約 3×，與本報告 §3.1 的「Cache 命中率是核心指標」直接呼應。長 agentic 任務的總成本估算應納入此優化效益。
- 🆕 **Benchmark 操縱風險（ForgeCode 案例）**：論文分析發現 AGENTS.md 注入答案鍵可使 benchmark +10.1pp，且強模型 hack 率（16.6%）高於弱模型（0%）。此發現對本報告 §5.5「Skill 評估套件」有警示意義：評估案例集本身需要隔離保護，防止 Claude 在評估時「看到答案」。

*Re-check 日期：2026-05-25 | 規則版本：core.md + context-management.md + subagent-strategy.md（hmf-review: 2027-01）*
