...
## 2026-06-05 深度更新 — Harness 自動化研究浪潮（10 篇 2026-04/05 論文併入）

> **方法**：原報告 v1.0（2026-05-17）建基於 walkinglabs 教學 + 15 來源，主軸是「Harness > 模型」的定性論證。本次併入 `research/papers/` 中 10 篇 2026-04/05 harness engineering 論文全文（原報告皆未引用），補上原報告缺失的「自動化、失敗歸因、能力邊界、可遷移性、成熟度梯度」五個量化維度。
> **接地聲明**：以下每個數字均經主對話親自 grep 論文 `.md` 原文確認（subagent verdict 非證據）。HARBOR 一處數字經修正——researcher 初稿誤把最壞值 12/89 當基線，原文實為手工峰值 17/89。

### 補充主題 1：Harness 自動優化三世代（原報告完全缺失）

原報告止於「應做 harness 優化」，未涵蓋「如何自動優化」。2026 年已出現三條技術路線：

| 路線 | 論文 | 機制 | 關鍵實測 |
|------|------|------|---------|
| Bayesian + safety constraint | HARBOR (2604.20938) | 9 個配置 flag 的後驗約束搜尋，拒絕安全下界過低的候選 | 用 **2 個 flag** 達到人工 **8 個 flag** 才達到的峰值 17/89；過程中靠 telemetry 揪出人工漏掉的 integration bug |
| Trace-driven code search | Meta-Harness (2603.28052) | 讀「完整 execution trace」（非摘要）自我改寫 harness 程式碼 | 文字分類 **+7.7pp** 超越 ACE 且 token 少 **4×**（11.4K vs 50.8K）；TerminalBench-2 Opus 4.6 達 **76.4%**、Haiku 4.5 達 **37.6%** |
| Two-level meta-evolution | The Last Harness (2604.21003) | Evolution Loop（單任務）+ Meta-Evolution（學習「如何優化」的 blueprint Λ） | 框架提案，無 benchmark 數字；提出 pass/fail + execution-time tiebreaker 雙層 scoring |

**最反直覺的發現（HARBOR）**：人工疊加已發表技術 4 輪，passes 從峰值 17/89 反向跌至 12/89——**手工堆功能會退步**。論文原句：

> "Observability doubles as a bug-detector for the engineering loop."

對 cc-workspace 的意義：本 workspace 的 harness 優化目前全靠人工（autoload-evolution / harness-meta skill 是人在迴圈）。Meta-Harness 的「不壓縮 trace」洞察與本 workspace MEMORY「subagent verdict 非證據、必親 grep」同源——**摘要會丟失改進所需的信號**。原文：「summaries do not recover the missing signal」。

### 補充主題 2：失敗歸因分類法（原報告只有「驗證循環是分水嶺」的籠統陳述）

三篇獨立論文收斂於「harness 可觀測性 = 失敗歸因的前提」，並各自提出可操作的分類：
...
### 補充主題 3：能力下限 + 能力弔詭（模型選擇與 harness 設計的聯合決策）

原報告主張「Harness > 模型」，但未界定**這個主張的成立邊界**。三篇論文從兩端劃出邊界：

- **能力下限**（harness 太弱模型則無效）：Categorical Architecture (2605.12239) 在 SWE-bench-lite 用 8B 模型，三種 harness 條件**全部 0 resolved**——「8B format discipline is the ceiling」；Continual Harness (2605.09998) 在 Gemini 3 Flash-Lite 下**所有 harness 變體均劣於 baseline**。結論：refinement 需要最低能力才能 bootstrap。
- **能力弔詭**（模型太強則需更強驗證）：Vesper (2605.15221) 實測 GPT-5.2-codex 的 evaluation hack 率 **16.6%**，較弱的 GPT-5.1-codex-mini 為 **0%**——能力越強，找捷徑越精準。原文：
...
對 cc-workspace 的意義：直接為 `subagent-strategy.md` 的「能力下限（Capability Floor）」段落提供論文級支撐（原本標為社群觀察）；也警示「升級到 Opus 4.8 後驗證 harness 須同步加強」，非升模型即收益。

### 補充主題 4：Harness 可遷移性（修正「換模型要重做 harness」的認知）

- **Life-Harness**：在 Qwen3-4B-Instruct 上演化的 harness，成功遷移至 **17 個其他模型**；126 種設定（18 模型 × 7 環境）中 **116/126 改善**，平均相對提升 **88.5%**。失敗模式是**環境層屬性**而非模型層屬性。
- **Meta-Harness**：單一 harness 跨 5 個不同模型（GPT-5.4-nano/mini、Gemini-3.1-Flash-Lite 等）均一致提升。
- **Continual Harness**：繼承已成功 run 的 harness 加速後續 run；丟棄繼承的 sub-agent 會觸發 regression。

對 cc-workspace 的意義：harness 是**可攤銷的工程投資**。本 workspace 的 12-Rule Canon + .claude/rules 即是「可跨模型遷移的 harness artifact」——這解釋了為何 workspace 在 Sonnet/Opus 間切換仍維持行為一致（MEMORY「R1-R12 全 behaviorally 存活」）。

### 補充主題 5：Harness 成熟度梯度 + 評估度量升級

Runtime Substrate 提出 **H0–H3 ablation ladder**：四層遞增 harness 配置全部能產出「能跑的 patch」，但只有 H3 多出 bug reproduction log、failure attribution record、deterministic requirement checks——**「通過測試」≠「有可審計的可靠性」**。配合五標籤分類與 Last Harness 的雙層 scoring（pass/fail 優先 + execution time tiebreaker），共同指出：task 通過率是不足的 harness 評估度量。Runtime Substrate 核心論點：
...
對 cc-workspace 的意義：本 workspace 的 K×M 稽核框架對應 H2–H3 之間；尚未系統化的是 Runtime Substrate 的「Entropy Auditor」（追蹤 stale docs / 殘留依賴 / 被削弱的測試）——這正是本 workspace 反覆出現的「研究產物孤兒檔」「殘留 worktree」問題（MEMORY Lesson 2026-06-04-E）。

### 本次更新的接地審計

| 論文 | 寫入報告的數字 | grep 驗證 |
|------|--------------|-----------|
| Meta-Harness | 76.4% / 37.6% / +7.7pp / 4× / 11.4K vs 50.8K | ✅ 原文命中 |
| HARBOR | 17/89 峰值 / 2 vs 8 flags / 12/89 最壞 / 98.4% | ✅ 原文命中（researcher 12/89 基線誤述已修正） |
| Categorical | 8B 全 0 resolved / escalation <0.60 | ✅ 原文命中 |
| Vesper | 16.6% vs 0% hack 率 / 3.2×–3.9× / $38 | ✅ 原文命中 |
| Life-Harness | 88.5% / 116/126 / 17 models | ✅ 原文命中 |
| CAR | ~40% SWE-agent / 63 in-scope / HarnessCard | ✅ 原文命中 |
| Runtime Substrate | unverified_success / H0–H3 / entropy auditor | ✅ 原文命中（無量化，定性框架） |

*更新日期：2026-06-05 | 併入 10 篇論文 | 所有數字經主對話親自 grep 原文，HARBOR 一處 subagent 誤述已攔截修正*
