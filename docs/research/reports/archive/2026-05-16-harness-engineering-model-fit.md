...
## 2026-06-05 增量論文併入（AgentOpt + configuring-agentic，主對話親自 grep 接地）

> **去重說明**：姊妹報告 `2026-05-17-harness-engineering.md` 同日併入 10 篇 harness 論文（HARBOR / Meta-Harness / Vesper / Life-Harness / Runtime-Substrate 等），涵蓋自動優化、失敗歸因、能力下限/弔詭、可遷移性、成熟度梯度五維。本節**不複述**該批數字，只聚焦兩篇對「配置選擇 / 模型適配」維度的獨特貢獻。

### 補充 1：in-the-wild 配置實證校準（configuring-agentic，arXiv:2602.14690）— 真 gap

首個大規模實證研究（**2,926 個 GitHub 倉庫**），為本報告通篇框架分析提供缺失的外部效度：

- **CLAUDE.md 45.4% + AGENTS.md 40.6%**：cc-workspace 雙軌策略與野外最高採用率吻合，非過度客製；AGENTS.md 在新建倉庫加速成長（market convergence 向跨工具標準）。
- **Skills 83.3% 靜態退化**：野外 Skills 中 **83.3%** 為純靜態文件，僅 **16.7%** 具動態工具呼叫。§6.2「Overexploration Trap」描述的靜態退化是社群普遍模式；附錄 B 七項品質清單（allowed-tools / Plan-Validate-Execute / HarnessCard）有實證必要性，workspace 的 research-hub / autoresearch 等結構化執行流 Skills 屬野外前 16.7% 少數。
- **Claude Code 配置機制廣度全工具最高**：論文識別 8 種機制（Context Files / Skills / Memory Files / Rules Files / MCP Servers / Hooks / Permission Sets / Environment Variables），cc-workspace 全數使用符合 power-user 實證模式。

**接地審計**：`2,926` ← "Analyzes 2,926 GitHub repositories" (2026-02-16-configuring-agentic…md L20)；`45.4%`/`40.6%` ← "CLAUDE.md (45.4%) and AGENTS.md (40.6%)" (L20/L58/L59)；`83.3%`/`16.7%` ← "83.3% of 'Skills' in the wild are static documentation, not dynamic executable flows" (L20/L82) + "Dynamic… 16.7%" (L83)；廣度最高 ← "Claude Code | Highest (uses broadest range)" (L69)；成長趨勢 ← "AGENTS.md adoption has grown faster than CLAUDE.md in newer repositories" (L124)。

### 補充 2：UCB-E eval budget + 角色量化（AgentOpt，arXiv:2604.06296）— narrow top-up（§5.2）
...
*增量併入日期：2026-06-05 | configuring-agentic（真 gap，外部效度）+ AgentOpt（narrow top-up）| 與姊妹報告去重，所有數字經主對話親自 grep 原文*
