...
**症狀**：sub-agent 回傳錯誤或不完整，直覺歸咎「模型不夠好」，換模型後重現。

**根因（empirical）**：coordination-arch（arXiv:2605.03310）實測 multi-agent 系統生產失敗率 **41–87%**，協調缺陷佔多數而非模型能力不足；顯式 hierarchical 協調可壓到 **12–18%**。四大失敗分類：

| 失敗類 | 佔比 | 對應本報告既有規範 |
|--------|------|------------------|
| Information inconsistency（各 agent 看到不同 context） | 34% | §4.2 Distractor — fork 前注入關鍵架構決策到每個 sub-agent prompt |
| Decision duplication（重複做同件事） | 28% | §5.3 Sequential Dependency — 委派前確認任務邊界互不重疊 |
| Responsibility ambiguity（無交接協議） | 24% | §4.4 Child 失敗無回報 — prompt 明寫「產出交給哪層、誰判斷完成」 |
| Race conditions（無同步機制） | 14% | §5.3 — 序列依賴改序列執行，平行任務確認 outputs 無交叉寫入 |
...
**接地審計**：`41–87%` / `12–18%` ← verbatim quote (2026-05-05-coordination-architectural-layer…md L181)；`34/28/24/14%` ← failure breakdown 表 (同檔 L56–59，summary table 非 verbatim quote，框架成立不依賴精確數字)。

### 🆕 GOTCHA #44：Sub-agent 過度彙整導致私有資訊外洩（非注入型）

**症狀**：sub-agent 讀取含 credentials / PII / 業務邏輯的檔案後，回傳「完整背景摘要」，無意把 task-irrelevant 私有資訊帶出。

**根因（conceptual）**：CalBench（arXiv:2605.09823）揭示 multi-agent 的 **coordination-privacy trade-off**——agent 須共享足夠資訊才能協調，但過度共享洩露私有語境。與 §4.5 prompt injection（被外部惡意**注入**）是不同方向：此處是 agent **主動輸出** task-irrelevant 私有資訊。
